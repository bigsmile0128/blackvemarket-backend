...
exports.takeOwnership = async (req, res) => {
    ...
    try {
        ...
        for (const nft of nft_list) {
            ...
            const to = await getTokenOwner(address, token_id);
            if (to) {
    ...
    try {
        ...
        for (var i = 0; i < nfts.length; i++) {
            ...
            if (auctions_by_token_id[nft["token_id"]]) {
                ...
                const highestOffer = await Offers.findOne({
                    auction_id: auctions_by_token_id[nft["token_id"]]["_id"],
                })
                    .sort("-price")
                    .lean()
                    .exec();
                nft["offer"] = highestOffer;
                nft["price"] = highestOffer
                    ? nft["offer"]["price"]
                    : nft["auction"]["price"];
                nfts[i] = nft;
            }
        ...
        if (sort == 0) {
            ...
            nfts = _.orderBy(
                nfts,
                [(o) => (o.auction ? o.auction.startedAt : 0)],
                "desc"
            );
        }
    ...
    try {
        ...
        if (auction) {
            isAuctionLive =
                ...
                auction["startedAt"] + auction["duration"] > timeNow;
        }
        ...
        for (var i = 0; i < offers.length; i++) {
            const userOffer = await User.findOne({ address: offers[i].buyer })
                .lean()
                .exec();
...
exports.createCollection = (req, res) => {
    const { name, symbol, description, address, user_id, total_supply } =
        req.body;
    ...
    try {
        ...
        for (const auction of auctions) {
            ...
            const details = await nftModel
                ...
                .lean()
                .exec();
            const offer = await Offers.findOne({ auction_id: auction["_id"] })
                .sort("-price")
                .lean()
                .exec();
            liveAuctions.push({
